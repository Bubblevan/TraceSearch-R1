"""Small CPU/CUDA-compatible fallback for rotary embeddings."""
...
def _position_ids(seqlen: int, batch: int, seqlen_offsets: Union[int, torch.Tensor], *, device: torch.device) -> torch.Tensor:
...
def _apply_rotary_block(x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor, *, interleaved: bool, conjugate: bool) -> torch.Tensor:
    ...
    x_rot, x_tail = x[..., :rotary_dim], x[..., rotary_dim:]
    if interleaved:
        x_even, x_odd = x_rot[..., ::2], x_rot[..., 1::2]
        rotated = torch.stack((x_even * cos - x_odd * sin, x_odd * cos + x_even * sin), dim=-1).flatten(-2)
    else:
        x_first, x_second = torch.chunk(x_rot, 2, dim=-1)
        rotated = torch.cat((x_first * cos - x_second * sin, x_second * cos + x_first * sin), dim=-1)
...
def apply_rotary(
    ...
    if cu_seqlens is None:
        ...
        selected_cos = selected_cos.reshape(batch, seqlen, -1) if positions.ndim == 2 else selected_cos.reshape(seqlen, -1).unsqueeze(0)
        selected_sin = selected_sin.reshape(batch, seqlen, -1) if positions.ndim == 2 else selected_sin.reshape(seqlen, -1).unsqueeze(0)
        output = _apply_rotary_block(x, selected_cos, selected_sin, interleaved=interleaved, conjugate=conjugate)
    else:
        ...
        for index in range(boundaries.numel() - 1):
            ...
            offset = seqlen_offsets if isinstance(seqlen_offsets, int) else int(seqlen_offsets[index])
            positions = torch.arange(end - start, device=x.device, dtype=torch.long) + offset
